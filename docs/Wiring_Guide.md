...
> 现场联调的实际接线进度、已验证链路和坏件/疑似坏件记录，见 `docs/Field_Integration_Log.md`。
...
| ESP32 AT 模块 | TX、RX、EN、RST | D8 (PA9, TX1)、D2 (PA10, RX1)、3.3 V、PC3（可选复位） | 当前现场联调优先使用 `GPIO23(TX)` → `D2(PA10)`、`GPIO18(RX)` → `D8(PA9)`，AT 固件命令口波特率为 `57600`；若使用带 USB 口的开发板，请在“USB 供电”和“外部 3.3 V 供电”之间二选一，不要同时上电 |
...
6. **ESP32 模块**：当前现场优先使用 4 根杜邦线连接 `TX(GPIO23)→PA10(RX1, D2)`、`RX(GPIO18)→PA9(TX1, D8)`、`VCC→3.3 V`、`GND→GND`。如果需要硬件复位，可将 EN 脚拉高，将 RST 接到 STM32 PC3（可选）。若你使用的是带 `CH340/CP210x` 的 `ESP32` 开发板并且已经插着自己的 `USB` 线，则不要再把 `VCC/3V` 接到 `NUCLEO 3V3`，只保留 `GND` 共地和 `TX/RX` 信号线。旧的 `GPIO17/16` 方案在本轮现场中会出现“先 ONLINE 再掉 OFF”的不稳定现象，当前不再作为首选。
...
6. ESP32 若已通过自己板载 USB 口供电，不要再同时连接 `NUCLEO 3V3`；避免双路供电或回灌；  
7. 泵与加热棒与外界水接触处做好绝缘，电源线没有裸露；  
8. OLED、温度探头等外设固定牢靠，避免运行时拉扯。  
