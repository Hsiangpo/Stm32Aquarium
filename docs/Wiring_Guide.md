...
| ESP32 AT 模块 | TX、RX、EN、RST | D8 (PA9, TX1)、D2 (PA10, RX1)、3.3 V、PC3（可选复位） | 供电 3.3 V，若模块自带稳压，请确认默认电压 |
...
6. **ESP32 模块**：使用 4 根杜邦线连接 TX(GPIO17)→PA10(RX1, D2)、RX(GPIO16)→PA9(TX1, D8)、VCC→3.3 V、GND→GND；如果需要硬件复位，可将 EN 脚拉高，将 RST 接到 STM32 PC3（可选）。
